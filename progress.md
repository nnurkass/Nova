...
| 5.2 | Realtime SSE / WebSocket Стриминг & Web Dashboard | ✅ Завершён |
...
- [x] Шаг 5.2 — Realtime SSE / WebSocket стриминг и Web Dashboard (`frontend/`)
- [x] Шаг 5.3 — Тестовое покрытие (133 теста, 85% coverage)
- [ ] Шаг 5.4 — Мониторинг и логирование (LangSmith)
- [ ] Шаг 5.5 — Деплой в production

---

## Заметки по этапу 5.2 (Realtime Streaming & Web Dashboard) (2026-08-26)

### 1. Бэкенд Стриминг (SSE & WebSocket)
- `nova/graph/main_graph.py`: генератор `stream_pipeline(task)` для гранулярной трансляции событий каждого узла графа (`pipeline_start`, `coo`, `procurement`, `pto`, `supply`, `coo_summary`, `pipeline_complete`).
- `nova/api/routes/tasks.py`:
  - `GET /api/v1/tasks/{task_id}/stream`: SSE стриминг с сохранением истории и воспроизведением.
  - `POST /api/v1/tasks/stream`: SSE стриминг запуска задачи на лету.
  - `WS /api/v1/tasks/{task_id}/ws` и `/ws/tasks/{task_id}`: WebSocket стриминг.
- `tests/integration/test_streaming.py`: 4 интеграционных теста для SSE и WebSocket.

### 2. Frontend Dashboard (`frontend/`)
- Стек: React 18 + Vite 6 + TypeScript + Tailwind CSS + Lucide Icons.
- **Главный экран & Task Creator**: пресеты тендеров РК (Алматы, Астана, Шымкент, Караганда), выбор региона, фильтр бюджета.
- **Realtime Pipeline Visualizer**: 5-этапный интерактивный граф агентов с анимацией статусов (`Pending` / `Running` / `Done` / `Error`).
- **Agent Terminal**: терминал потока мыслей агентов в реальном времени с цветовыми бейджами и автопрокруткой.
- **Вкладка 1 — Тендер**: номер, заказчик, БИН, бюджет в KZT, карточка скоринга 100/100, лоты.
- **Вкладка 2 — Ведомость ПТО**: ведомость объемов работ (ВОР) и ресурсная ведомость материалов (+5% технологический запас).
- **Вкладка 3 — Складской баланс**: KPI остатков, статус дефицита, таблица склада с прогресс-барами.
- **Вкладка 4 — Заявки на закупку (PO)**: группировка по поставщикам РК, цены в ₸, +10% страховой буфер, бейджи `🔴 URGENT`.
- **Вкладка 5 — Финансовый дашборд**: выручка, затраты, валовая маржа %, диаграмма распределения затрат, Markdown-отчет COO.
- **История задач**: боковая панель с возможностью мгновенного просмотра и воспроизведения любого сохраненного запуска.
- **Экспорт**: выгрузка в Markdown (.md), печать в PDF, копирование в буфер обмена.
...
```
- **133 passed** (все 129 существующих + 4 новых теста стриминга)
- **Фронтенд собирается без ошибок** (0 TS lints, bundle 259 kB)
